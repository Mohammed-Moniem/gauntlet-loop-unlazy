...
const SCRIPTS = [
  ...
  "tests/hybrid-contract-tests.mjs",
...
check("the PLAN template carries a revisioned contract denominator", () => {
  ...
  const missing = required.filter((token) => {
    const haystack = /[a-z]/.test(token) && token === token.toLowerCase() ? lowerPlan : plan;
...
check("request reconciliation keeps the focused solo cheap path", () => {
  ...
  if (!/a PLAN table is not required/i.test(skill)) return "focused solo path was made needlessly orchestrated";
