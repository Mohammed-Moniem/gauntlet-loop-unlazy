...
Codex host surfaces differ. Use the native nonblocking subagent or worker tools available in the current host, and obey that host's rules about when user-visible tasks may be created. If no native nonblocking worker is available for subtasks, record a sequential fallback and do not claim a parallel wave.

When the host exposes explicit launch and wait tools, follow this semantic order:

1. call the native launch tool once for each leaf in the open wave
2. record each returned agent id with `dispatch-check start`
3. seal the wave
4. wait or read results only after seal
